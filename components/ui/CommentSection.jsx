...
export default function CommentSection({ topicId, title = "Comments" }) {
  ...
  // Fitur Baru: Balas Komentar (Reply)
  const [replyTo, setReplyTo] = useState(null);
  // State untuk melacak popup reaction mana yang terbuka
  ...
  // Emoji Khusus untuk React Komentar
  const reactEmojis = [
    "😀",
    "😂",
    "🥰",
    ...
    const { error } = await supabase.from("comments").insert([
      {
        ...
        user_role: user.user_metadata?.role || "user", // Kirim Role untuk Badge
        topic_id: topicId,
        content: newComment,
        image_url: finalImageUrl,
        parent_id: replyTo ? replyTo.id : null, // ID parent jika membalas
      },
    ]);
  ...
  const toggleReaction = async (comment, emoji) => {
    ...
    // 3. Bersihkan emoji jika tidak ada yang bereaksi
    ...
    // 4. Update state UI agar langsung berubah (Optimistic Update)
    setComments(
      comments.map((c) =>
        c.id === comment.id ? { ...c, reactions: currentReactions } : c,
      ),
    );
    setActiveReactComment(null); // Tutup popup

    // 5. Update ke Supabase
    const { error } = await supabase
      .from("comments")
      .update({ reactions: currentReactions })
      .eq("id", comment.id);

    // 6. Tampilkan peringatan jika Supabase menolak
    if (error) {
      console.error("Supabase Error Update Reaction:", error);
      alert("Gagal menyimpan reaction! (Cek RLS Supabase)");
      fetchComments(); // Kembalikan ke data asli jika gagal
    }
  };

  // KOMPONEN RENDER KOMENTAR REKURSIF (Untuk Sistem Threading)
  const CommentItem = ({ comment, isReply = false }) => {
    // Ambil semua komentar yang parent_id nya adalah komentar ini
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Aksi Bawah: Reply, React, dan Daftar Reaction Terpilih */}
          <div className="flex flex-wrap items-center gap-3 relative">
            <button
              ...
              onClick={(e) => {
                ...
                // Animasi Scroll Halus ke Kotak Input
            ...
            {/* Tampilkan Reaction yang sudah diberikan */}
            {comment.reactions &&
              Object.entries(comment.reactions).map(([emoji, users]) => (
                <button
                  key={emoji}
                  onClick={() => toggleReaction(comment, emoji)}
                  className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-bold border transition-colors ${users.includes(user?.id) ? "bg-celestia-pink/20 border-celestia-pink text-celestia-pink" : "bg-white/5 border-white/10 text-gray-400 hover:bg-white/10"}`}
                >
          ...
          {/* Render Anak-anak Komentar (Balasan) */}
  ...
  // Filter hanya komentar UTAMA (yang bukan balasan) untuk dirender di tingkat terluar
  const parentComments = comments.filter((c) => !c.parent_id);

  return (
    <div className="mt-12 bg-[#0D0B1A] rounded-2xl border border-white/5 p-6 md:p-8 shadow-xl animate-fade-in">
      {/* AREA INPUT KOMENTAR */}
      <div className="bg-[#151226]/80 rounded-2xl border border-white/10 p-1 mb-10 shadow-inner">
        {user ? (
          <form onSubmit={handleSubmit} className="relative p-4 md:p-5">
            {/* Header: Nickname & Indikator Reply */}
            <div className="flex flex-col md:flex-row md:items-center justify-between text-[11px] md:text-xs text-gray-400 mb-4 pb-4 border-b border-dashed border-white/10 gap-3">
              <div className="flex items-center gap-2">
                ...
                {user.user_metadata?.role === "admin" && (
                  <span className="bg-celestia-gold/20 text-celestia-gold text-[9px] font-black uppercase px-2 py-0.5 rounded">
                    👑 Admin MangNime
                  </span>
                )}
              </div>
            ...
            <div className="flex flex-wrap justify-between items-center mt-4 pt-4 border-t border-white/5 gap-4 relative">
              <div className="flex items-center gap-4">
                {/* 1. Emoji Picker Full (Dari Library Asli) */}
      ...
      <div className="space-y-4">
        {comments.length === 0 ? (
          ...
          // Hanya render parent (Komentar utama), anak-anaknya akan dirender secara rekursif di dalam CommentItem
